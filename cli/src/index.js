...
async function setupCursor(kitRoot, dest, answers) {
    ...
    // Always copy Project Standards & Roles (if selected)
    await fs.copy(path.join(skillsSrc, 'project-standards'), path.join(skillsDest, 'project-standards'));

    if (answers.includeRoles) {
        await fs.copy(path.join(skillsSrc, 'role-implementer'), path.join(skillsDest, 'role-implementer'));
        await fs.copy(path.join(skillsSrc, 'role-architect'), path.join(skillsDest, 'role-architect'));
        await fs.copy(path.join(skillsSrc, 'role-reviewer'), path.join(skillsDest, 'role-reviewer'));
        await fs.copy(path.join(skillsSrc, 'role-debugger'), path.join(skillsDest, 'role-debugger'));
    }
    ...
    // Always copy DevOps skills
    const dockerSrc = path.join(skillsSrc, 'devops-docker');
    if (await fs.pathExists(dockerSrc)) await fs.copy(dockerSrc, path.join(skillsDest, 'devops-docker'));
}
...
async function setupWindsurf(kitRoot, dest, answers) {
    ...
    // Always copy Project Standards & Roles
    await fs.copy(path.join(skillsSrc, 'project-standards'), path.join(skillsDest, 'project-standards'));
